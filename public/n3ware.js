...
(function (global) {
  ...
  class N3UI {
    ...
    static injectStyles() {
      ...
      s.textContent = [
        ...
        `.n3-cust-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9999997;display:flex;align-items:center;justify-content:center;animation:n3-fade-in .15s ease}`,
        `.n3-cust-modal{background:#0B1120;border:1px solid ${T.border};border-radius:14px;width:min(500px,94vw);max-height:90vh;display:flex;flex-direction:column;box-shadow:0 24px 80px rgba(0,0,0,.7);overflow:hidden}`,
        `.n3-cust-header{display:flex;align-items:center;gap:10px;padding:16px 18px;border-bottom:1px solid ${T.border};flex-shrink:0}`,
        `.n3-cust-icon{width:32px;height:32px;border-radius:8px;background:rgba(227,19,55,.15);border:1px solid rgba(227,19,55,.25);display:flex;align-items:center;justify-content:center;color:#E31337;flex-shrink:0}`,
        `.n3-cust-title{flex:1;font:600 14px/1 system-ui,sans-serif;color:${T.text}}`,
        `.n3-cust-close{background:transparent;border:none;color:${T.muted};cursor:pointer;width:26px;height:26px;border-radius:5px;display:flex;align-items:center;justify-content:center;transition:all .12s;flex-shrink:0}`,
        `.n3-cust-close:hover{background:rgba(255,255,255,.1);color:${T.text}}`,
        `.n3-cust-body{padding:18px;flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:14px}`,
        `.n3-cust-label{font:600 11px/1 system-ui,sans-serif;text-transform:uppercase;letter-spacing:.06em;color:${T.muted};margin-bottom:6px}`,
        `.n3-cust-textarea{width:100%;box-sizing:border-box;background:rgba(255,255,255,.05);border:1px solid ${T.border};color:${T.text};border-radius:8px;padding:10px 12px;font:13px/1.55 system-ui,sans-serif;outline:none;resize:vertical;min-height:80px;transition:border-color .15s;font-family:inherit}`,
        `.n3-cust-textarea:focus{border-color:${T.accent}}`,
        `.n3-cust-textarea::placeholder{color:${T.muted}}`,
        `.n3-cust-dropzone{border:2px dashed ${T.border};border-radius:8px;padding:18px;text-align:center;cursor:pointer;transition:border-color .15s,background .15s;position:relative}`,
        `.n3-cust-dropzone:hover,.n3-cust-dropzone.n3-cust-drag-over{border-color:${T.accent};background:rgba(227,19,55,.05)}`,
        `.n3-cust-dropzone-label{font:13px/1.5 system-ui,sans-serif;color:${T.muted}}`,
        `.n3-cust-dropzone-label span{color:${T.accent};text-decoration:underline;cursor:pointer}`,
        `.n3-cust-thumbs{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}`,
        `.n3-cust-thumb{position:relative;width:64px;height:64px;border-radius:6px;overflow:hidden;border:1px solid ${T.border}}`,
        `.n3-cust-thumb img{width:100%;height:100%;object-fit:cover}`,
        `.n3-cust-thumb-rm{position:absolute;top:2px;right:2px;width:16px;height:16px;border-radius:50%;background:rgba(0,0,0,.7);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;font-size:10px;line-height:1;padding:0}`,
        `.n3-cust-footer{display:flex;align-items:center;gap:8px;padding:14px 18px;border-top:1px solid ${T.border};flex-shrink:0}`,
        `.n3-cust-spacer{flex:1}`,
        `.n3-cust-skip{background:rgba(255,255,255,.07);border:1px solid ${T.border};color:${T.text};padding:8px 18px;border-radius:7px;cursor:pointer;font:600 13px/1 system-ui,sans-serif;transition:all .12s}`,
        `.n3-cust-skip:hover{background:rgba(255,255,255,.14)}`,
        `.n3-cust-generate{background:#E31337;border:1px solid #E31337;color:#fff;padding:8px 18px;border-radius:7px;cursor:pointer;font:600 13px/1 system-ui,sans-serif;transition:all .12s;display:flex;align-items:center;gap:6px}`,
        `.n3-cust-generate:hover{background:#FF2847;border-color:#FF2847}`,
        `.n3-cust-generate:disabled{opacity:.5;cursor:not-allowed}`,
        `.n3-cust-error{background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:7px;padding:10px 12px;font:13px/1.5 system-ui,sans-serif;color:#FCA5A5}`,
        // ── Placement mode ───────────────────────────────────────────────────
        `.n3-place-cursor{position:fixed;pointer-events:none;z-index:9999999;width:40px;height:40px;border-radius:9px;background:#E31337;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(227,19,55,.5);transition:opacity .1s;user-select:none}`,
        `.n3-place-cursor-badge{position:absolute;top:-5px;right:-5px;width:14px;height:14px;border-radius:50%;background:#fff;color:#E31337;font:700 10px/14px system-ui;display:flex;align-items:center;justify-content:center}`,
        `.n3-place-zone{position:absolute;left:0;right:0;height:4px;pointer-events:none;z-index:9999998;border-radius:2px;background:rgba(227,19,55,.22);transition:height .12s,background .12s,box-shadow .12s}`,
        `.n3-place-zone.n3-place-zone-active{height:4px;background:#E31337;box-shadow:0 0 12px #E31337;animation:n3-zone-pulse 1s ease-in-out infinite}`,
        `@keyframes n3-zone-pulse{0%,100%{box-shadow:0 0 10px #E31337}50%{box-shadow:0 0 20px #E31337,0 0 4px rgba(255,255,255,.4)}}`,
        `body.n3-placing{cursor:none!important}`,
        `body.n3-placing *{cursor:none!important}`,
        // ── Script placeholders ─────────────────────────────────────────────
  ...
  class N3Editor {
    ...
    _wireEvents() {
      ...
      this.events.on('component:insert', () => {
        this._snapshot();
    ...
    _handleKeyDown(e) {
      ...
      if (e.key === 'Escape') {
        // Let placement mode consume Escape before falling through to deactivate
        if (this.components && this.components.cancelPlacement && this.components.cancelPlacement()) return;
        this._deactivate();
      }
    }
