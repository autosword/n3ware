...
/**
 * Compose a page from a template's `sections` array.
 * Each section is one of:
 *   { type: 'component', id: '...' }  — resolved from componentsLib
 *   { type: 'custom',    html: '...' } — used verbatim
 *   { type: 'ai',        brief: '...'} — generated by Claude (small targeted call)
 *
 * @param {object}   template       Full template object with a `sections` array
 * @param {object[]} componentsLib  Array of component objects (from components.json)
 * @param {string}   description    User's business/page description
 * @param {string[]} imageUrls      Optional uploaded images
 * @param {string}   name           Final page title
 * @returns {Promise<string>}       Assembled HTML body
 */
async function composePage(template, componentsLib, description, imageUrls, name) {
  const parts = [];
  for (const section of template.sections) {
    if (section.type === 'component') {
      const comp = (componentsLib || []).find(c => c.id === section.id);
      if (comp && comp.html) {
        parts.push(comp.html);
      } else {
        console.warn(`[composePage] component "${section.id}" not found in library`);
      }
    } else if (section.type === 'custom') {
      parts.push(section.html);
    } else if (section.type === 'ai') {
      const aiHtml = await generateSectionWithAI(section.brief, description, imageUrls, name);
      parts.push(aiHtml);
    }
  }
  return parts.join('\n');
...
/**
 * Generate a single <section>…</section> HTML block via Claude.
 * Used for `type: 'ai'` sections in composition templates.
 *
 * @param {string}   brief       Short description of what this section should contain
 * @param {string}   description Business/page context from the user
 * @param {string[]} imageUrls   Optional images to incorporate
 * @param {string}   name        Page name (for context)
 * @returns {Promise<string>}    HTML section
 */
async function generateSectionWithAI(brief, description, imageUrls, name) {
  const anthropicKey = process.env.ANTHROPIC_API_KEY;

  if (!anthropicKey) {
    console.warn('[page-generator] ANTHROPIC_API_KEY not set — using mock section');
    return _mockSection(brief, description);
  }

  const imageNote = imageUrls && imageUrls.length
    ? `\nImages available to use: ${imageUrls.map((u, i) => `Image ${i + 1}: ${u}`).join(', ')}`
    : '';

  const prompt = `Generate a single HTML section using Tailwind CSS utility classes for a page called "${name}".

Business/page context: ${description}${imageNote}

Section brief: ${brief}

Requirements:
- Return ONLY the <section>...</section> HTML — no markdown, no explanation, no code fences
- Use Tailwind CSS for all styling (assume Tailwind CDN is present)
- Mobile-responsive with sm:/md:/lg: prefixes where helpful
- Professional, clean design consistent with a small business website`;
  ...
  try {
    ...
    console.error('[page-generator] generateSectionWithAI error:', err.message);
    return _mockSection(brief, description);
  }
}

function _mockSection(brief, description) {
  const title = brief.split(':')[0].replace(/^[^a-zA-Z]+/, '').slice(0, 60);
  const safe  = s => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  return `<section class="py-20 bg-gray-50">
  <div class="max-w-4xl mx-auto px-6 text-center">
    <h2 class="text-3xl font-bold text-gray-900 mb-6">${safe(title)}</h2>
    <p class="text-gray-600 text-lg leading-relaxed">${safe(description)}</p>
  </div>
</section>`;
}

module.exports = { generatePageWithAI, generateMockPage, addPageToNav, loadPageTemplate, customizeTemplateWithAI, composePage, generateSectionWithAI };
